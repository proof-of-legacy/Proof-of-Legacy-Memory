...
static double polm_measure_dram_ns(uint64_t nonce) {
    ...
    for (int i = 0; i < 20; i++) {
        idx = (idx * 6364136223846793005ULL + 1) % num_slots;
        (void)dag64[idx * stride_u64];
    }

    /* Pointer-chasing com rdtscp — sem syscall, resolução de ciclo */
    uint64_t c0 = rdtscp_read();
    for (int i = 0; i < 200; i++) {
    ...
    while (running) {
        ...
        while (running && !chain_moved) {
            ...
            if (posma_meets_target(result.hash_final, difficulty)) {
                ...
                char submit_url[512];
                snprintf(submit_url, sizeof(submit_url), "%s/v2/submit", node_url);
